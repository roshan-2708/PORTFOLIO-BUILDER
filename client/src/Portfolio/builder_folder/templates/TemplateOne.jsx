...
const TemplateOne = ({ data }) => {
    // Agar data nahi hai toh kuch render mat karo
    if (!data) return null;

    const [isMenuOpen, setIsMenuOpen] = useState(false);

    // FIX: Agar data.user exist karta hai (Slug view), toh use lo. 
    // Warna agar data direct hai (Preview/Admin view), toh use lo.
    ...
    return (
        <div className="min-h-screen bg-[#030712] text-slate-200 font-sans selection:bg-indigo-500/30">
            ...
            <nav className="fixed top-0 w-full z-50 border-b border-slate-800/50 bg-[#030712]/80 backdrop-blur-md">
                ...
                {/* Mobile Menu Overlay */}
                {isMenuOpen && (
                    <div className="md:hidden bg-[#030712] border-b border-slate-800 p-6 space-y-4 animate-in slide-in-from-top duration-300">
                        {navLinks.map((link) => (
                            <a key={link.name} href={link.href} onClick={() => setIsMenuOpen(false)} className="block text-lg hover:text-indigo-400 transition-colors">
                                {link.name}
                            </a>
                        ))}
                    </div>
            ...
            <section id="about" className="relative pt-40 pb-20 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-12 items-center">
                    <div className="lg:col-span-7 space-y-8">
                        ...
                        <h1 className="text-5xl md:text-7xl font-extrabold text-white leading-tight">
                            I'm <span className="text-indigo-500">{displayUser?.firstName || "Developer"}</span>, <br />
                            {title}
                        </h1>
            ...
            <section id="projects" className="py-24 px-6 relative">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid md:grid-cols-2 gap-10">
                        {projects.map((project, i) => (
                            <div key={i} className="group relative bg-slate-900/50 border border-slate-800 rounded-[2.5rem] overflow-hidden hover:border-indigo-500/50 hover:bg-[#0f172a] transition-all duration-500 shadow-xl">
                                <div className="relative h-72 md:h-80 overflow-hidden">
                                ...
                                <div className="p-10">
                                    ...
                                    <p className="text-slate-400 mt-4 text-sm line-clamp-3">{project.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
